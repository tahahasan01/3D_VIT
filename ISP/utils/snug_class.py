...
class Deformed_Cloth:
    def __init__(self,):
        self.f = None
        self.v = None
        self.n = None

    def update(self, verts_batch, faces_batch, indicator_v, indicator_f):
        self.v = verts_batch
        self.f = faces_batch
        n = []
        for i in range(len(verts_batch)):
            self.v[i, ~indicator_v[i]] = self.v[i, [0]].clone()
            self.f[i, ~indicator_f[i]] = self.f[i, [0]].clone()
            mesh = trimesh.Trimesh(self.v[i].detach().cpu().numpy(), self.f[i].detach().cpu().numpy(), process=False, validate=False)
            n.append(mesh.vertex_normals)

        self.n = torch.FloatTensor(n).cuda()

    def update_single(self, verts_batch, faces_batch):
        self.v = verts_batch
        self.f = faces_batch

        mesh = trimesh.Trimesh(self.v[0].detach().cpu().numpy(), self.f[0].detach().cpu().numpy(), process=False, validate=False)
        self.n = torch.FloatTensor(mesh.vertex_normals).cuda().unsqueeze(0)
...
class FaceNormals:
    ...
    def call(self, vertices, faces):
        ...
        if self.normalize:
            face_normals = face_normals/torch.norm(face_normals, p=2, dim=-1, keepdim=True)
