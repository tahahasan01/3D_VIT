...
# ---------------------------------------------------------------------------
# Chumpy-free pickle loading — SMPL .pkl files contain serialised chumpy
# arrays.  When chumpy is not installed we inject a lightweight shim
# so that ``pickle.load`` can deserialise them as plain numpy arrays.
# ---------------------------------------------------------------------------

def _ensure_chumpy_shim() -> None:
    """Insert a tiny ``chumpy`` mock into ``sys.modules`` if the real
    package is not installed.  This lets ``pickle.load`` deserialise
    chumpy objects inside old SMPL .pkl files without the actual package.
    """
    if "chumpy" in sys.modules:
        return
    try:
        import chumpy  # noqa: F401 – real package available, nothing to do
        return
    except ImportError:
        pass
    ...
    class _FakeCh:
        """Lightweight stand-in for chumpy.Ch / chumpy.ch.Ch.

        Pickle reconstructs these via ``__new__`` + ``__setstate__``.
        We just store the state dict and expose ``.r`` which returns the
        underlying numpy data (stored under the ``x`` key by chumpy).
        """
...
def _load_smpl_pickle(path: str) -> dict:
    """Load SMPL pkl, replacing chumpy arrays with numpy on the fly."""
    _ensure_chumpy_shim()
    with open(path, "rb") as f:
        data = pickle.load(f, encoding="latin1")
    # Convert chumpy-like objects → plain ndarray
    for k, v in list(data.items()):
        if not isinstance(v, (np.ndarray, str, int, float, list, dict, tuple, type(None))):
            if hasattr(v, "r"):
                data[k] = np.asarray(v.r)
...
from .lbs import (lbs, vertices2joints, blend_shapes, lbs_smplicit, lbs_custom)
...
class SMPL(nn.Module):
    ...
    def __init__(self, model_path, data_struct=None,
        ...
        if data_struct is None:
            ...
            with open(smpl_path, 'rb') as smpl_file:
                data_struct = Struct(**_load_smpl_pickle(smpl_path))
    ...
    def forward_custom(self, betas=None, body_pose=None, global_orient=None,
            transl=None, return_verts=True, return_full_pose=False,displacement=None,v_template=None,
            **kwargs):
        ''' Forward pass for the SMPL model
        ...
        if displacement is not None:
            vertices, joints_smpl, T_weighted, W, T, pose_offsets, shape_offsets = lbs_custom(betas, full_pose, v_template+displacement,
                                   self.shapedirs, self.posedirs,
                                   self.J_regressor, self.parents,
                                   self.lbs_weights, dtype=self.dtype,pose_blend=self.pose_blend)
        else:
            vertices, joints_smpl, T_weighted, W, T, pose_offsets, shape_offsets = lbs_custom(betas, full_pose, v_template,
                                        self.shapedirs, self.posedirs,
                                        self.J_regressor, self.parents,
                                        self.lbs_weights, dtype=self.dtype,pose_blend=self.pose_blend)
